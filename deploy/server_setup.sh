...
# the server and drops configs/magicboxie.example.yaml there as a starting
...
#
# config/magicboxie.yaml holds secrets (auth.password_hash, tmdb.api_read_token)
...
#   vim /var/www/magicboxie.com/config/magicboxie.yaml
#
# To generate the password hash, run it through the image itself once an
# image exists at ghcr.io/kriogenx0/magicboxie-web (i.e. after the first CI
# run/push to main):
#   docker run --rm ghcr.io/kriogenx0/magicboxie-web hash-password '<your password>'
#
# movies_dir/music_dir/data_dir in the example already point at the
# container-internal paths (/content/movies, /content/music,
# /var/lib/magicboxie) that docker-compose.prod.yml's named volumes back --
...
if ssh "$SSH_TARGET" "test -f '$REMOTE_DIR/config/magicboxie.yaml'"; then
  echo "==> $REMOTE_DIR/config/magicboxie.yaml already exists -- leaving it alone"
else
  echo "==> Seeding config/magicboxie.yaml from the example"
  scp "$SCRIPT_DIR/../configs/magicboxie.example.yaml" "$SSH_TARGET:$REMOTE_DIR/config/magicboxie.yaml"
  echo "==> Edit it now: ssh $SSH_TARGET, then vim $REMOTE_DIR/config/magicboxie.yaml"
